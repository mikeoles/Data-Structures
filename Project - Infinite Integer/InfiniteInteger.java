...
public class InfiniteInteger implements Comparable<InfiniteInteger>
{
	...
	public InfiniteInteger minus(final InfiniteInteger anInfiniteInteger)
	{
		...
		for(int i = larger.infNumber.length-1; i >= difference; i--)	
		{
			...
			else
			{
				...
				if(larger.infNumber[i-1] == 0)
				{
					int loc = i;
					while(larger.infNumber[loc-2] == 0)
					{
						larger.infNumber[loc-1] = 9;
						loc--;
					}
					larger.infNumber[loc-1] = 9;		
					larger.infNumber[loc-2] = larger.infNumber[loc-2] - 1;					
	 ...
	 */
	public InfiniteInteger multiply(final InfiniteInteger bottomInteger)
	{
		// TO DO
		//multiplication		
		boolean negative = true;
		if(this.isNeg && bottomInteger.isNeg)
		{
			negative = false;
		}
		if(!this.isNeg && !bottomInteger.isNeg)
		...
		InfiniteInteger topInteger = new InfiniteInteger(0);		
		topInteger = new InfiniteInteger(this.toString());		
		
		InfiniteInteger[] row;
		row = new InfiniteInteger[bottomInteger.infNumber.length];			
		String zeroes = "";
		
		for(int i=bottomInteger.infNumber.length-1; i>=0; i--)
		{			
			int carryOver = 0;
			String line = "";		
			for(int j=topInteger.infNumber.length-1; j>=0; j--)
			{
				int result = bottomInteger.infNumber[i]*topInteger.infNumber[j]+carryOver;
				if(result<10)
				{
					line = Integer.toString(result) + line;
					carryOver = 0;
				}
				else
				{
				
					line = Integer.toString(result%10) + line;
					carryOver = result/10;
				}
			}
			if(carryOver != 0)
			...
			line = line + zeroes;
			row[i] = new InfiniteInteger(line);	
			zeroes = zeroes + "0";
		}	
		...
		{
			total = total.plus(row[i]);
		}
					
		if(negative && !topInteger.toString().equals("0") && !bottomInteger.toString().equals("0"))
		{
			total = new InfiniteInteger("-" + total.toString());	
		}
